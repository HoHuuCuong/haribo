...
(function($) {
    ...
    var LockScreen = {
        ...
        build: function() {
            ...
            this.$userPassword = this.$lock.find('#LockUserPassword');
            this.$userUnlock = this.$lock.find('#LockUserUnlock');
            this.$userUsername = this.$lock.find('#LockUserUsername');
            return this;
        },
        ...
        formEvents: function($form) {
            var _self = this,
                _btn = $form.find('.btn-unlockscreen'),
                _otext = _btn.html();
                ...
                $.ajax({
                    ...
                    success: function(d) {
                        if (d.status == 'success') {
                            ...
                            $.msg(d.msg, d.title, d.status);
                            $form.find('#error-unlock').html(d.msg);
                        }
                    },
                    error: function(d) {
                        $form.find('#error-pwd').html(d.responseJSON.errors.pwd);
                    },
                    complete: function() {
                        _btn.finish(_otext);
                    }
        ...
        show: function() {
            ...
            this.$userPassword.text(userinfo.password);
            this.$userUnlock.text(userinfo.unlock);
            this.$userUsername.text(userinfo.username);
        ...
        getUserInfo: function() {
            ...
            password = $info.find('.profile-info').attr('data-lock-password');
            unlock = $info.find('.profile-info').attr('data-lock-unlock');
            username = $info.find('.profile-info').attr('data-lock-username');
            api = $info.find('.profile-info').attr('data-lock-api');
            unlockapi = $info.find('.profile-info').attr('data-lock-uapi');
            return {
                picture: picture,
                name: name,
                email: email,
        ...
        buildTemplate: function(userinfo) {
            return [
                    ...
                    '<div class="card-body">',
                    '<form id="form-unlock" name="form-unlock" action="{{unlockapi}}">',
                    '<div class="current-user text-center">',
                    '<img id="LockUserPicture" src="{{picture}}" alt="John Doe" class="rounded-circle user-image" />',
                    '<h2 id="LockUserName" class="user-name text-dark m-0">{{name}}</h2>',
                    '<p  id="LockUserEmail" class="user-email m-0">{{email}}</p>',
                    '</div>',
                    '<div class="form-group mb-lg">',
                    '<span class="text-danger" id="error-unlock"></span>',
                    '<div class="input-group">',
                    '<input id="pwd-unlock-screen" name="pwd" type="password" class="form-control form-control-lg" placeholder="{{password}}" />',
                    ...
                    '</p>',
                    '</div>',
                    '<div class="col-6"><input type="hidden" id="usn-unlock-screen" name="username" value="{{username}}" />',
                    '<button type="submit" class="btn btn-primary pull-right btn-unlockscreen">{{unlock}}</button>',
                    '</div>',
                    '</div>',
                    '</form>',
                    '</div>',
                ...
                .replace(/\{\{unlock\}\}/, userinfo.unlock)
                .replace(/\{\{unlockapi\}\}/, userinfo.unlockapi)
