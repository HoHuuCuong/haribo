                        ...
                        case 'INPUT':
                            if (vv.required) {
                                if (!vv.value.trim()) {
                                    ...
                                    $.msg($m.data('msg'),'Lỗi nhập liệu','danger');
                                }
...
jQuery(document).ready(function($) {
  ...
  $('.search').on('keyup',function(){
        var _that = $(this),_val = _that.val(),_kq = $('.ketqua');
        if(_val)
        {
            $('.item-rs').each(function(){
                if($(this).text().toUpperCase().indexOf(_val.toUpperCase())>-1)
                {
                    $(this).show();
                }else{
                    $(this).hide();
